...
@section('content')
    <div class="w-full max-w-full flex-none px-3">
        ...
        @endif
        @if (session('error'))
            <div class="relative mb-4 w-full rounded-lg border border-solid border-emerald-300 bg-gradient-to-tl from-red-600 to-orange-600 p-4 text-white"
                autoalert alert-duration="5000">
        ...
        <div
            ...
            <div
                ...
                <h6 class="m-0 dark:text-white">Lends List</h6>
                ...
                <div class="mt-3 flex w-full items-center lg:mt-0 lg:w-3/6">
                    <form class="ease relative flex w-full flex-wrap items-stretch rounded-lg transition-all"
                        action={{ route('admin.lends.index') }} method="get">
                        ...
                        <input
                            ...
                            type="text" placeholder="Lender (User) Name" name="q" autocomplete="off" />
                        {{-- </div> --}}
                    </form>
                        ...
                        <tbody>
                            @foreach ($lends as $lend)
                                <tr>
                                    ...
                                    <td
                                        class="whitespace-nowrap border-b bg-transparent p-2 align-middle shadow-transparent dark:border-white/40">
                                        <form id="deleteForm-{{ $lend->id }}"
                                            action="{{ route('admin.lends.destroy', $lend->id) }}" method="post">
                                            @csrf
                                            @method('delete')
                                        </form>
                                        <form id="returnForm-{{ $lend->id }}"
                                            action="{{ route('admin.lends.return', $lend->id) }}" method="post">
                                            @csrf
                                        </form>
                                        <button
                                            class="inline-block whitespace-nowrap rounded-1.8 bg-gradient-to-tl from-emerald-600 to-teal-500 px-2.5 py-1.4 text-center align-baseline text-xs font-bold uppercase leading-none text-white"
                                            id="return" type="button" item-id="{{ $lend->id }}">
                                            Return
                                        </button>
                                        <a class="inline-block whitespace-nowrap rounded-1.8 bg-gradient-to-tl from-blue-500 to-violet-500 px-2.5 py-1.4 text-center align-baseline text-xs font-bold uppercase leading-none text-white"
                                            href="{{ route('admin.lends.edit', $lend->id) }}">
                                            Edit
                                        </a>
                                        <button
                                            ...
                                            id="delete" type="button" item-id="{{ $lend->id }}">
                                            Delete
                                        </button>
    ...
    <script>
        ...
        document.querySelectorAll('#return').forEach(item => {
            item.addEventListener('click', event => {
                const itemId = item.getAttribute('item-id');
                event.preventDefault();
                document.getElementById(`returnForm-${itemId}`).submit();
            })
        })
    </script>
@endsection
